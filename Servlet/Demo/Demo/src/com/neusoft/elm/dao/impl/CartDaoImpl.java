...
public class CartDaoImpl implements CartDao {
    ...
    public int removeCart(Cart cart) throws Exception {
        int result = 0;
        StringBuilder sql = new StringBuilder("delete from cart where userId=? and businessId=? ");
        if (cart.getFoodId() != null) {
            sql.append(" and foodId=").append(cart.getFoodId());
        }
        try {
            con = DBUtil.getConnection();
    ...
    public List<Cart> listCart(Cart cart) throws Exception {
        ...
        if (cart.getBusinessId() != null) {
            sql.append(" and c.businessId=").append(cart.getBusinessId());
        }
        try {
            con = DBUtil.getConnection();
            pst = con.prepareStatement(sql.toString());
